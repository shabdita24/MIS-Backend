...
import com.mis.eoffice.db4Repo.AppointmentDisplayNameRepositoryBMRL;
...
public class DetailedTableServiceImplBMRL {
	...
	@Autowired
	private AppointmentDisplayNameRepositoryBMRL appointmentDisplayNameRepository;

	@Autowired
	private FileFolderNameRepositoryBMRL fileFolderNameRepository;
	...
	public ResponseDetailTable getdetailedtableInboxFile(String sauName, Integer num,String command,Integer pageNo, Integer rows) {
		...
		if (sauDat.isPresent()) {
			...
			List<FileInventoryBMRL> hd1 = filerepo.findByInitiatedBySauAndMisTypeAndTaskState(sauName, "FILE", status,page);
			List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskState(sauName, "FILE", status);
			size=hd2.size();			
			if (hd1.size() > 0) {
				for (int i = 0; i < hd1.size(); i++) {
					...
					FileInventoryBMRL fm2 = hd1.get(i);
					...
					String iniBy = "";
					Optional<FileFolderNameInventoryBMRL> fileName = fileFolderNameRepository.findByFileName(file);
					...
					Optional<AppointmentDisplayNameInventoryBMRL> apptName = appointmentDisplayNameRepository
	...
	private List<DetailedTable> filesOfBranches(List<HierarchyDataInventory> sauSubBranches, List<DetailedTable> dt,
		...
		for (HierarchyDataInventory branches : sauSubBranches) {
			...
			if (typeOfFunction == 0) {
				List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskState(branches.getSauName(),
						"FILE", status);
				...
				List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBetween(
				...
				List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBefore(
				...
				Date lastDate = Date.from(lastDate1.atStartOfDay(ZoneId.systemDefault()).toInstant());
				Date currentDate = Date.from(currentDate1.atStartOfDay(ZoneId.systemDefault()).toInstant());
				List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedAfter(
						branches.getSauName(), "FILE", status, lastDate);
	...
	private List<DetailedTable> branchesViaType(List<FileInventoryBMRL> hd2, List<DetailedTable> dt, String command) {
		
		if (hd2.size() > 0) {
			for (int i = 0; i < hd2.size(); i++) {
				DetailedTable obj = new DetailedTable();
				logger.info("filename == " + hd2.get(i).getFileNumber());
				FileInventoryBMRL fm2 = hd2.get(i);
				Optional<HierarchyDataInventory> ss = htrepo.findBySauNameAndCommand(fm2.getInitiatedBySau(),command);
				String depName ="";
				if(ss.isPresent()) {
					 depName = ss.get().getDepFullName();
	...
	public ResponseDetailTable getdetailedtablepend37(String sau, Integer num,String command, Integer pageNo, Integer rows) {
		...
		int size=0;
		Pageable page = PageRequest.of(pageNo, rows);
		List<FileInventoryBMRL> sauData = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBetween(sau,
				"FILE", status, sevendaysbefore, threedaysbefore,page);
		List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBetween(sau,
		...
		if (sauDat.isPresent()) {
			...
			if (sauDataFilter.size() > 0) {
				for (int i = 0; i < sauDataFilter.size(); i++) {
					...
					FileInventoryBMRL fm2 = sauDataFilter.get(i);
					Optional<HierarchyDataInventory> ss = htrepo.findBySauNameAndCommand(fm2.getInitiatedBySau(),command);
					String depName ="";
					if(ss.isPresent()) {
						 depName = ss.get().getDepFullName();
...
//	}
	...
	public ResponseDetailTable getdetailedtablepro30days(String sau, Integer num,String command, Integer pageNo, Integer rows) {
		...
		int size=0;
		Pageable page = PageRequest.of(pageNo, rows);
		List<FileInventoryBMRL> hd1 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBefore(sau,
				"FILE", taskstate, lastDate);
		List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBefore(sau,
				"FILE", taskstate, lastDate);
		size=hd2.size();
		Optional<HierarchyDataInventory> sauDat = htrepo.findBySauNameAndCommand(sau,command);
		if (sauDat.isPresent()) {
	...
	public ResponseDetailTable getdetailedtablependingat1020(String sauName, Integer num,String command,Integer pageNo, Integer rows) {
		...
		int size=0;
		Pageable page = PageRequest.of(pageNo, rows);
		List<FileInventoryBMRL> hd1 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBetween(sauName,
				"FILE", status, sevendaysbefore, threedaysbefore,page);
		List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBetween(sauName,
				"FILE", status, sevendaysbefore, threedaysbefore);
		size=hd2.size();
		Optional<HierarchyDataInventory> sauDat = htrepo.findBySauNameAndCommand(sauName,command);
		if (sauDat.isPresent()) {
	...
	public ResponseDetailTable getdetailedtablependingatten(String sau, Integer num,String command, Integer pageNo, Integer rows) {
		...
		int size=0;
		Pageable page = PageRequest.of(pageNo, rows);
		List<FileInventoryBMRL> hd1 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBefore(sau, "FILE", taskstate, lastDate,page);
		List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBefore(sau,
				"FILE", taskstate, lastDate);
		size=hd2.size();
		Optional<HierarchyDataInventory> sauDat = htrepo.findBySauNameAndCommand(sau,command);
...
//		Date currentDate = Date.from(currentDate1.atStartOfDay(ZoneId.systemDefault()).toInstant());
		...
		int size=0;
		Pageable page = PageRequest.of(pageNo, rows);
		List<FileInventoryBMRL> hd1 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedAfter(sau, "FILE", taskstate, lastDate,page);
		List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedAfter(sau,"FILE", taskstate, lastDate);
		size=hd2.size();
		Optional<HierarchyDataInventory> sauDat = htrepo.findBySauNameAndCommand(sau,command);
		if (sauDat.isPresent()) {
//			List<FileInventory> hd1=filerepo.findByInitiatedBySauAndMisTypeAndTaskState(sauName,"FILE",status);??
	...
	public ResponseDetailTable getdetailedtablependingGreater20(String sau, Integer num,String command, Integer pageNo, Integer rows) {
...
//		Date currentDate = Date.from(currentDate1.atStartOfDay(ZoneId.systemDefault()).toInstant());
		...
		int size=0;
		Pageable page = PageRequest.of(pageNo, rows);
		List<FileInventoryBMRL> hd1 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedAfter(sau,
				"FILE", taskstate, lastDate,page);
		List<FileInventoryBMRL> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedAfter(sau,
				"FILE", taskstate, lastDate);
		size=hd2.size();
		Optional<HierarchyDataInventory> sauDat = htrepo.findBySauNameAndCommand(sau,command);
		if (sauDat.isPresent()) {
