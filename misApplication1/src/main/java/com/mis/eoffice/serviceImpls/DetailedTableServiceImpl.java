...
public class DetailedTableServiceImpl implements DetailedTableService {
	...
	public ResponseDetailTable getdetailedtablependingat1020(String sauName, Integer num,String command,Integer pageNo, Integer rows) {
		...
		int size=0;
		Pageable page = PageRequest.of(pageNo, rows);
		List<FileInventory> hd1 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBetween(sauName, "FILE", status,sevendaysbefore, threedaysbefore,page);
	...
	public ResponseDetailTable getdetailedtablependingatten(String sau, Integer num,String command,Integer pageNo, Integer rows) {
		...
		int size=0;
		Pageable page = PageRequest.of(pageNo, rows);
		List<FileInventory> hd1 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBefore(sau, "FILE", taskstate, lastDate,page);
		List<FileInventory> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedBefore(sau,
	...
	public ResponseDetailTable getdetailedtablependingatfive(String sau, Integer num,String command, Integer pageNo, Integer rows) {
		...
		int size=0;
		Pageable page = PageRequest.of(pageNo, rows);
		List<FileInventory> hd1 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedAfter(sau, "FILE", taskstate, lastDate,page);
		List<FileInventory> hd2 = filerepo.findByInitiatedBySauAndMisTypeAndTaskStateAndDateTimeRecievedAfter(sau,"FILE", taskstate, lastDate);
		size=hd2.size();
		Optional<HierarchyDataInventory> sauDat = htrepo.findBySauNameAndCommand(sau,command);
		if (sauDat.isPresent()) {
