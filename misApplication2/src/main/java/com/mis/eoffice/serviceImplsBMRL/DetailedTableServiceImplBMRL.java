...
public class DetailedTableServiceImplBMRL {
	...
	private List<DetailedTable> filesOfBranches(List<HierarchyDataInventory> sauSubBranches, List<DetailedTable> dt,
		...
		for (HierarchyDataInventory branches : sauSubBranches) {
			...
			if (typeOfFunction == 0) {
				List<FileInventoryBMRL> hd2 = filerepo.findByCurrDepAndMisTypeAndTaskState(branches.getSauName(),
						"FILE", status);
				...
				List<FileInventoryBMRL> hd2 = filerepo.findByCurrDepAndMisTypeAndTaskStateAndDateTimeRecievedBetween(
				...
				List<FileInventoryBMRL> hd2 = filerepo.findByCurrDepAndMisTypeAndTaskStateAndDateTimeRecievedBefore(
				...
				Date lastDate = Date.from(lastDate1.atStartOfDay(ZoneId.systemDefault()).toInstant());
				Date currentDate = Date.from(currentDate1.atStartOfDay(ZoneId.systemDefault()).toInstant());
				List<FileInventoryBMRL> hd2 = filerepo.findByCurrDepAndMisTypeAndTaskStateAndDateTimeRecievedAfter(
						branches.getSauName(), "FILE", status, lastDate);
