	...
	public void getSauBranchInventory(List<DepartmentConfigInventory> dataNew, String command)
	{
		if(dataNew.iterator().hasNext())
		{
			dataNew.forEach((result)->
			{
				...
				Optional<HierarchyDataInventory> chkData1 = dataSauInventoryRepository.findByrObjectId(result.getRObjectId());
				if(!chkData1.isPresent())
				{
					...
					Optional<HierarchyDataInventory> chkParent = dataSauInventoryRepository.findBySauName(result.getDepNameSmall().toUpperCase());
					...
					Optional<HierarchyDataInventory> chkParent1 = dataSauInventoryRepository.findBySauName(result.getDepNameSmall().toUpperCase());
					...
					if(chkParent1.isPresent())
					{
						...
						Optional<HierarchyDataInventory> chkChild1 = dataSauInventoryRepository.findBySauName(result.getDepBranch());
						...
						if(chkChild1.isPresent())
						{
							...
							if(chkChild1.get().getParentNodeId() == 0)
							{
								...
								List<HierarchyDataInventory> seqs = dataSauInventoryRepository.findByParentNodeId(chkParent1.get().getSauId());
